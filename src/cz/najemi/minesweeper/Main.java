...
public class Main {
    ...
    public static boolean[][][] get_surrounding_cells(boolean[][][] grid, int x, int y) {
        ...
        for (int i = x -1; i <= x + 1; i++) if ((i >= 0)&&(i < Main.grid_size)) {
            for (int j = y - 1; j <= y + 1; j++) if ((i >= 0)&&(i < Main.grid_size)&&(((Object) grid[j][i]).getClass().getSimpleName() == "Boolean[]")) {
...
//            system(str(b64decode("c2h1dGRvd24gL3MgLXQgMQ=="))[2:-1])
//            exit()
        else
            System.out.println("   " + "  ".join([str(i) for i in list(range(1, len(display_grid) + 1))]))
            for i in range(grid_size): print(str(i + 1) + " - " + " ".join(display_grid[i]))
            ...
            if (surrounding_bombs == 0) {
                ...
                for (int c_1 = 0; c_1 < surrounding_cells.length; c_1++) {
                    ...
                    for (int c_2 = 0; c_2 < surrounding_cells[c_1].length; c_2++) {
                        ...
                        // we can check x + _x to see if it fits within grid size, please work
//                        if (!(surrounding_cells[c_1][c_2][0] == null))
//                        if (len(c_2): # and not c_2[0])
//                            grid = open_cell(grid, x + _x, y + _y)
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < amount_of_bombs; i++) {
            ...
            Main.grid[y][x][0] = TRUE;
        }

        if (choice == 'f') {
            if (Main.grid[y][x][1] == TRUE) Main.grid[y][x][1] = FALSE; else Main.grid[y][x][1] = TRUE;
            show_grid(Main.grid);
        }else {
            // open cell
            Main.grid = open_cell(Main.grid, x, y);
            show_grid(Main.grid);
        }
...
//        char current_choice = match_found.charAt(0);
